...
const BOARD_TILES = [
  ...
  { pos:4,  type:'tax',      label:'INCOME TAX',   icon:'💸',  value:300 },
  { pos:5,  type:'railroad', name:'Station',       price:300,  color:'RAILROAD' },
  { pos:6,  type:'prop',     name:'Bhubaneswar',   price:180,  color:'LIGHT_BLUE' },
  { pos:7,  type:'chance',   label:'CHANCE',       icon:'🎴' },
  { pos:8,  type:'prop',     name:'Panaji',        price:200,  color:'LIGHT_BLUE' },
  { pos:9,  type:'prop',     name:'Vadodara',      price:240,  color:'LIGHT_BLUE' },
  { pos:10, type:'corner',   label:'JAIL',         icon:'🔒',  corner:'jail' },
  { pos:11, type:'prop',     name:'Patna',         price:300,  color:'PINK' },
  { pos:12, type:'utility',  name:'Electric',      price:200,  color:'UTILITY' },
  { pos:13, type:'prop',     name:'Bhopal',        price:320,  color:'PINK' },
  { pos:14, type:'prop',     name:'Indore',        price:360,  color:'PINK' },
  { pos:15, type:'railroad', name:'Station',       price:300,  color:'RAILROAD' },
  { pos:16, type:'prop',     name:'Nagpur',        price:400,  color:'ORANGE' },
  { pos:17, type:'chest',    label:'CHEST',        icon:'📦' },
  { pos:18, type:'prop',     name:'Kochi',         price:440,  color:'ORANGE' },
  { pos:19, type:'prop',     name:'Lucknow',       price:480,  color:'ORANGE' },
  { pos:20, type:'corner',   label:'FREE PARKING', icon:'🅿️',  corner:'park' },
  { pos:21, type:'prop',     name:'Ahmedabad',     price:520,  color:'RED' },
  { pos:22, type:'chance',   label:'CHANCE',       icon:'🎴' },
  { pos:23, type:'prop',     name:'Hyderabad',     price:560,  color:'RED' },
  { pos:24, type:'prop',     name:'Pune',          price:600,  color:'RED' },
  { pos:25, type:'railroad', name:'Station',       price:300,  color:'RAILROAD' },
  { pos:26, type:'prop',     name:'Jaipur',        price:640,  color:'YELLOW' },
  { pos:27, type:'prop',     name:'Chandigarh',    price:680,  color:'YELLOW' },
  { pos:28, type:'utility',  name:'Water Works',   price:200,  color:'UTILITY' },
  { pos:29, type:'prop',     name:'Lucknow',       price:720,  color:'YELLOW' },
  { pos:30, type:'corner',   label:'GO TO JAIL',   icon:'👮',  corner:'gotojail' },
  { pos:31, type:'prop',     name:'Chennai',       price:760,  color:'GREEN' },
  { pos:32, type:'prop',     name:'Kolkata',       price:800,  color:'GREEN' },
  { pos:33, type:'chest',    label:'CHEST',        icon:'📦' },
  { pos:34, type:'prop',     name:'Bengaluru',     price:840,  color:'GREEN' },
  { pos:35, type:'railroad', name:'Station',       price:300,  color:'RAILROAD' },
  { pos:36, type:'chance',   label:'CHANCE',       icon:'🎴' },
  { pos:37, type:'prop',     name:'Delhi',         price:900,  color:'DARK_BLUE' },
  { pos:38, type:'tax',      label:'SUPER TAX',    icon:'💰',  value:150 },
  { pos:39, type:'prop',     name:'Mumbai',        price:1000, color:'DARK_BLUE' },
];
...
function renderBoardOwnership(state) {
  ...
  Object.values(state.props).forEach(prop => {
    ...
    if (prop.owner) {
      ...
      const color    = ownerIdx >= 0 ? PLAYER_COLORS[ownerIdx % 8] : '#888';
      if (prop.mortgaged) {
        // Diagonal stripe in owner colour — signals mortgaged at a glance
        strip.style.background =
          `repeating-linear-gradient(45deg,${color} 0px,${color} 3px,transparent 3px,transparent 6px)`;
    ...
    row.innerHTML = `
      ...
      <div class="build-btns">
        ...
        <button class="btn-build-action btn-mortgage"
          ...
          title="Mortgage for ₹${mortgageValue}"
          onclick="doMortgage(${prop.pos})">🏦 ₹${mortgageValue}</button>
        ` : `
        <button class="btn-build-action btn-unmortgage"
          ${me.money < unmortgageCost ? 'disabled' : ''}
          title="Lift mortgage for ₹${unmortgageCost}"
          onclick="doUnmortgage(${prop.pos})">↩ ₹${unmortgageCost}</button>
        `}
      </div>
    `;
...
function doMortgage(pos) {
  send('/mortgage', { gameId: currentGameId, playerId: myPlayerId, propertyPos: pos });
  addLog(`<span class="log-player">${myPlayerName}</span> <span class="log-action">🏦 mortgaged</span> <span class="log-prop">${getTileName(pos)}</span>`);
  closeModal('modal-build');
}

function doUnmortgage(pos) {
  send('/unmortgage', { gameId: currentGameId, playerId: myPlayerId, propertyPos: pos });
  addLog(`<span class="log-player">${myPlayerName}</span> <span class="log-action">↩ lifted mortgage on</span> <span class="log-prop">${getTileName(pos)}</span>`);
  closeModal('modal-build');
}
